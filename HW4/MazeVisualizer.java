...
public class MazeVisualizer extends JApplet {
	...
	private static int[] qColors = { 166, 206, 227, 31, 120, 180, 178, 223, 138, 51, 160, 44, 251, 154, 153, 227, 26,
			28, 253, 191, 111, 255, 127, 0, 202, 178, 214, 106, 61, 154, 255, 255, 153, 177, 89, 40 };
	...
	/**
	 * Creates a maze visualizer for an n x n square maze with no edges.
	 * 
	 * @param n
	 *            The number of vertices in the horizontal (and vertical) direction.
	 */
	...
	/**
	 * Performs any necessary initialization before drawing begins.
	 */
	public void init() {
		setBackground(bg);
		setForeground(fg);
	...
	/**
	 * Method to add an edge to be visualized. Each edge is visualized as a line. @
	 * param from The vertex where the edge starts. @ para to The vertex where the
	 * edge ends.
	 */
	...
	/**
	 * Method to add a path to be visualized. The path is specified as a list of
	 * integer nodes. The path is assumed to be a valid path in the maze. @ param
	 * nodeList List of nodes that make up the path
	 */
	...
	/**
	 * The paint method for this applet. Do not call this method, it is called
	 * automatically whenever the applet needs to draw its content.
	 */
	public void paint(Graphics g) {
		...
		for (Pair<Integer, Integer> edge : edgeList) {
			g2.draw(new Line2D.Double(lo + 1 + (edge.x - 1) / N, lo + 1 + (edge.x - 1) % N, lo + 1 + (edge.y - 1) / N,
					lo + 1 + (edge.y - 1) % N));
		}
		...
		if (pathList != null) {
			...
			for (List<Integer> path : pathList) {
				...
				for (int i = 0; i < path.size() - 1; i++) {
					g2.draw(new Line2D.Double(lo + 1 + (path.get(i) - 1) / N, lo + 1 + (path.get(i) - 1) % N,
							lo + 1 + (path.get(i + 1) - 1) / N, lo + 1 + (path.get(i + 1) - 1) % N));
				}
